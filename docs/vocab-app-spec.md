...
| 오답률 높은 단어 | 시도 이력이 1회 이상인 단어 중 오답률 내림차순 상위 N개 (동률이면 `wrongCount` 많은 순) | N = 20 (상수로 조정 가능) |
| 전체 단어 | 전체 단어, 순서는 매 세션 랜덤 셔플 | - |
...
- 오답률 높은 단어 테스트 기본 개수: 20개, 시도 이력 없는 단어는 제외
- 전체 단어 테스트: 매 세션 랜덤 셔플
